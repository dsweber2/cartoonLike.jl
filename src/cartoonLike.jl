...
"""
function given_points(points::Array{T}; cart::cartoonLikeImage{<:Float64} =
...
"""
function distinguishing_curvature(n; cart::cartoonLikeImage{<:Float64} =
                                  cartoonLikeImage()) 
  yRange = range(-1,1,length=2*n+1)
  points = make_points.(yRange)
  image_groups = given_points.(make_points.(yRange), cart =
                               cartoonLikeImage{Float64}())
